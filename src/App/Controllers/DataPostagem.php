...
class DataPostagem
{
    ...
    public function getTempoPostagem()
    {
        ...
        if ($dataAtual->format('Y') > $dataPostagem->format('Y')) {
            $mensagem = $dataAtual->format('Y') - $dataPostagem->format('Y') . ' anos atras';
        } elseif ($dataAtual->format('m') > $dataPostagem->format('m')) {
            $mensagem = $dataAtual->format('m') - $dataPostagem->format('m') . ' meses atras';
        } elseif ($dataAtual->format('d') > $dataPostagem->format('d')) {
            $mensagem = $dataAtual->format('d') - $dataPostagem->format('d') . ' dias atras';
        } elseif ($dataAtual->format('H') > $dataPostagem->format('H')) {
            $mensagem = $dataAtual->format('H') - $dataPostagem->format('H') . ' horas atras';
        } elseif ($dataAtual->format('i') > $dataPostagem->format('m')) {
            $mensagem = $dataAtual->format('i') - $dataPostagem->format('m') . ' minutos atras';
        } elseif ($dataAtual->format('s') > $dataPostagem->format('s')) {
            $mensagem = $dataAtual->format('s') - $dataPostagem->format('s') . ' segundos atras';
        }
        return $mensagem;
    }
}